...
def parse_args():
    ...
    parser.add_argument('image_path', help='the path of stare-img_dir.tar')
...
def main():
    ...
    mkdir_or_exist(osp.join(out_dir, 'img_dir', 'training'))
    mkdir_or_exist(osp.join(out_dir, 'img_dir', 'validation'))
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        ...
        print('Extracting stare-img_dir.tar...')
        ...
        for filename in sorted(os.listdir(now_dir))[:TRAINING_LEN]:
            ...
            mmcv.imwrite(
                img,
                osp.join(out_dir, 'img_dir', 'training',
        ...
        for filename in sorted(os.listdir(now_dir))[TRAINING_LEN:]:
            ...
            mmcv.imwrite(
                img,
                osp.join(out_dir, 'img_dir', 'validation',
                         osp.splitext(filename)[0] + '.png'))
