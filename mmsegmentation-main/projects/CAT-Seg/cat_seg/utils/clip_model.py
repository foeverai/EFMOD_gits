...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x):
        """
        Args:
            x (torch.Tensor): the input mini-batch img_dir.
        """
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        """Calculate ann_dir multi-head-attention."""
...
class CLIP(nn.Module):
    ...
    def build_attention_mask(self):
        """Create causal attention ann_dir."""
        # lazily create causal attention ann_dir, with full attention between
        # the vision tokens pytorch uses additive attention ann_dir; fill with
        # -inf
    ...
    def forward(self, image, text):
        """
        Args:
            image (torch.Tensor): input img_dir.
