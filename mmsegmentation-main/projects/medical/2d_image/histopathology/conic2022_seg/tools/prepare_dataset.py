...
def save_images_from_npz(data, save_root, part='img_dir/'):
    num = data.shape[0]
    for i in range(num):
        np_img = data[i, :, :, :]
...
images_npy = np.load('data/CoNIC_Challenge/img_dir.npy')
...
for ith, part in enumerate([x_train]):
    ...
    for img in part:
        ...
        shutil.copy(
            img, root_path + 'img_dir/' + part_dir + basename.split('.')[0] +
            save_img_suffix)
