...
def process_dataset(file_lists, part_dir_dict):
    for ith, part in enumerate(file_lists):
        ...
        for sample in part:
            ...
            # check image file
            img_save_path = os.path.join(root_path, 'img_dir', part_dir,
        ...
        # print image num
        part_dir_folder = os.path.join(root_path, 'img_dir', part_dir)
        print(
            f'{part_dir} has {len(os.listdir(part_dir_folder))} img_dir completed!'  # noqa
        )
...
if __name__ == '__main__':
    ...
    print(f'training img_dir: {len(train_imgs)}, test img_dir: {len(test_imgs)}')

    os.system('mkdir -p ' + root_path + 'img_dir/train/')
    os.system('mkdir -p ' + root_path + 'img_dir/test/')
