...
if not os.path.exists(os.path.join(path, 'img_dir', 'train')):
    os.system(f'mkdir -p {os.path.join(path, "img_dir", "train")}')
...
for img in imgs_amd14:
    shutil.copy(
        ...
        os.path.join(path, 'img_dir', 'train', img))
    # copy GT
...
imgs = os.listdir(os.path.join(path, 'img_dir', 'train'))
for img in imgs:
    if not img.endswith('.png'):
        im = Image.open(os.path.join(path, 'img_dir', 'train', img))
        im.save(os.path.join(path, 'img_dir', 'train', img[:-4] + '.png'))
