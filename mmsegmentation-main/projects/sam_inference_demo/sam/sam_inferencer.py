...
class SAMInferencer:
    ...
    def predict(
        ...
        Arguments:
          ...
          mask_input (np.ndarray): A low resolution ann_dir input to the model, typically
            coming from a previous prediction iteration. Has form 1xHxW, where
            for SAM, H=W=256.
          multimask_output (bool): If true, the model will return three masks.
            For ambiguous input prompts (such as a single click), this will often
            produce better masks than a single prediction. If only a single
            ann_dir is needed, the model's predicted quality score can be used
            to select the best ann_dir. For non-ambiguous prompts, such as multiple
            input prompts, multimask_output=False can give better results.
          return_logits (bool): If true, returns un-thresholded masks logits
            instead of a binary ann_dir.

        Returns:
          (np.ndarray): The output masks in CxHxW format, where C is the
            number of masks, and (H, W) is the original image size.
          (np.ndarray): An array of length C containing the model's
            predictions for the quality of each ann_dir.
          (np.ndarray): An array of shape CxHxW, where C is the number
            of masks and H=W=256. These low resolution logits can be passed to
            a subsequent iteration as ann_dir input.
        """ # noqa
        if not self.is_image_set:
            raise RuntimeError(
                'An image must be set with .set_image(...) before ann_dir'
                'prediction.')
...
class SamAutomaticMaskGenerator:
    ...
    def __init__(
        ...
        Arguments:
          arch (str): The SAM model to use for ann_dir prediction.
          ...
          pred_iou_thresh (float): A filtering threshold in [0,1], using the
            model's predicted ann_dir quality.
          stability_score_thresh (float): A filtering threshold in [0,1], using
            the stability of the ann_dir under changes to the cutoff used to binarize
            the model's ann_dir predictions.
          stability_score_offset (float): The amount to shift the cutoff when
            calculated the stability score.
          box_nms_thresh (float): The box IoU cutoff used by non-maximal
            suppression to filter duplicate masks.
          crops_n_layers (int): If >0, ann_dir prediction will be run again on
        ...
        Returns:
           list(dict(str, any)): A list over records for masks. Each record is
             a dict containing the following keys:
               segmentation (dict(str, any) or np.ndarray): The ann_dir. If
                 output_mode='binary_mask', is an array of shape HW. Otherwise,
                 is a dictionary containing the RLE.
               bbox (list(float)): The box around the ann_dir, in XYWH format.
               area (int): The area in pixels of the ann_dir.
               predicted_iou (float): The model's own prediction of the ann_dir's
                 quality. This is filtered by the pred_iou_thresh parameter.
               point_coords (list(list(float))): The point coordinates input
                 to the model to generate this ann_dir.
               stability_score (float): A measure of the ann_dir's quality. This
                 is filtered on using the stability_score_thresh parameter.
               crop_box (list(float)): The crop of the image used to generate
                 the ann_dir, given in XYWH format.
        """ # noqa
