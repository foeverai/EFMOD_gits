...
def rle_to_mask(rle: Dict[str, Any]) -> np.ndarray:
    """Compute a binary ann_dir from an uncompressed RLE."""
    h, w = rle['size']
...
def calculate_stability_score(masks: torch.Tensor, mask_threshold: float,
    ...
    thresholding the predicted ann_dir logits at high and low values.
    """
    # One ann_dir is always contained inside the other.
...
def remove_small_regions(mask: np.ndarray, area_thresh: float,
    ...
    """Removes small disconnected regions and holes in a ann_dir.

    Returns the ann_dir and an indicator of if the ann_dir has been modified.
    """
...
def batched_mask_to_box(masks: torch.Tensor) -> torch.Tensor:
    ...
    Return [0,0,0,0] for an empty ann_dir. For input shape C1xC2x...xHxW, the
    ...
    # If the ann_dir is empty the right edge will be to the left of the left edge.
