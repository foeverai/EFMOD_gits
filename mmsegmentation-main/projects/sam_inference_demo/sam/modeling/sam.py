...
class SAM(nn.Module):
    ...
    def __init__(
        ...
        Arguments:
          image_encoder (ViTSAM): The backbone used to encode the
            image into image embeddings that allow for efficient ann_dir
            prediction.
    ...
    def forward(
        ...
        """Predicts masks end-to-end from provided img_dir and prompts. If
        ...
        Arguments:
          batched_input (list(dict)): A list over input img_dir, each a
            ...
            excluded if it is not present.
              ...
              'mask_inputs': (torch.Tensor) Batched ann_dir inputs to the model,
                in the form Bx1xHxW.
          multimask_output (bool): Whether the model should predict multiple
            disambiguating masks, or return a single ann_dir.

        Returns:
          (list(dict)): A list over input img_dir, where each element is
            as dictionary with the following keys.
              'masks': (torch.Tensor) Batched binary ann_dir predictions,
                with shape BxCxHxW, where B is the number of input prompts,
                C is determiend by multimask_output, and (H, W) is the
                original size of the image.
              'iou_predictions': (torch.Tensor) The model's predictions
                of ann_dir quality, in shape BxC.
              'low_res_logits': (torch.Tensor) Low resolution logits with
                shape BxCxHxW, where H=W=256. Can be passed as ann_dir input
