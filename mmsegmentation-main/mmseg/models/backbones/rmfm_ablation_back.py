...
class RMFMNet_ablation_backon(BaseModule):
    def __init__(self,
                 in_channels=3,
                 base_channels=32,
        ...
        self.conv1 = DoubleConv(self.input_channel,out_channels[0],norm_cfg=norm_cfg,act_cfg=act_cfg)  #(bs,c,h,w)→(bs,32,h,w)
        ...
        self.decoder4 = FMCM2(out_channels[3]*2, out_channels[3]*2,norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.decoder3 = FMCM2(out_channels[2]*2, out_channels[2]*2,norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.decoder2 = FMCM1(out_channels[1]*2, out_channels[1]*2,norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.decoder1 = FMCM1(out_channels[0]*2, out_channels[0]*2,norm_cfg=norm_cfg,act_cfg=act_cfg)
        ...
        self.conv6 = Coa_conv(out_channels[3]*2, out_channels[3],norm_cfg=norm_cfg,act_cfg=act_cfg)   # (bs,512,h,w)→(bs,256,h/8,w/8)

        self.conv7 = Coa_conv(out_channels[2]*2, out_channels[2],norm_cfg=norm_cfg,act_cfg=act_cfg)   # (bs,256,h,w)→(bs,128,h/4,w/4)

        self.conv8 = Coa_conv(out_channels[1]*2, out_channels[1],norm_cfg=norm_cfg,act_cfg=act_cfg)   # (bs,128,h,w)→(bs,64,h/2,w/2)
...
if __name__ == '__main__':
    ...
    moduel = RMFMNet_ablation_backon(3).cuda()
