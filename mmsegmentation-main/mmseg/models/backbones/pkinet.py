...
class Star_block(BaseModule):
    ...
    def __init__(
            ...
            out_channels: int,
            expansion: float = 1.0,
            norm_cfg: Optional[dict] = dict(type='BN', momentum=0.03, eps=0.001),  # 归一化层的相关参数
            act_cfg: Optional[dict] = dict(type='SiLU'),
            init_cfg: Optional[dict] = None,
    ):
        super().__init__(init_cfg)
        hidden_channels = make_divisible(int(out_channels * expansion), 8)  # 计算隐藏层的大小，由于论文中使用的是8块GPU所以要保证隐藏层的通道数要能够被8整除
...
class InceptionBottleneck(BaseModule):  # 实现论文中的PKI Module和CAA Module的融合
...
class PKINet(BaseModule):
    ...
    def __init__(
            self,
            arch: str = 'S',  # 模型架构默认为s
            out_indices: Sequence[int] = (2, 3, 4),  # 输出的索引序列默认为(2,3,4)
            drop_path_rate: float = 0.1,  # dropout的概率为0.1
            frozen_stages: int = -1,      # 冻结的阶段数，默认为-1，不冻结任何层
            norm_eval: bool = False,      # 是否在评估模式下使用归一化，默认为false
            arch_setting: Optional[Sequence[list]] = None,  # 架构设置的序列，默认为NONE
            norm_cfg: Optional[dict] = dict(type='BN', momentum=0.03, eps=0.001),  # 归一化配置字典
            act_cfg: Optional[dict] = dict(type='SiLU'),  # 激活函数配置字典，默认为一个包含类型的字典
            init_cfg: Optional[dict] = dict(type='Kaiming',
                                            layer='Conv2d',
                                            a=math.sqrt(5),
                                            distribution='uniform',
                                            mode='fan_in',
                                            nonlinearity='leaky_relu'), # 初始化配置字典
    ):
        super().__init__(init_cfg=init_cfg)
        arch_setting = arch_setting or self.arch_settings[arch]  # 如果arch=None则使用self.arch_settings[arch]作为其值

        assert set(out_indices).issubset(i for i in range(len(arch_setting) + 1))  # 断言检查，第一个断言确保`out_indices`中的所有索引都是`arch_setting`的长度范围内的有效索引。
        if frozen_stages not in range(-1, len(arch_setting) + 1):                   # 第二个断言确保`frozen_stages`的值在有效范围内。
            raise ValueError(f'frozen_stages must be in range(-1, len(arch_setting) + 1). But received {frozen_stages}')

        self.out_indices = out_indices  # 赋值给对应的类对象
        self.frozen_stages = frozen_stages
        self.norm_eval = norm_eval
        self.stages = nn.ModuleList()  # 定义一个空的模型列表

        self.stem = Star_block(3, arch_setting[0][0], expansion=1.0, norm_cfg=norm_cfg, act_cfg=act_cfg)
