...
class MCB(BaseModule):
    def __init__(self,
        ...
        self.con = ConvModule(out_chan*4,out_chan,kernel_size=1,stride=1,padding=0,norm_cfg=None,act_cfg=act_cfg)
    ...
    def forward(self, x):
        ...
        x3 = self.mc3(x)
        x4 = self.incept(x)
        out = self.con(torch.cat([x1, x2, x3,x4], dim=1))
        return out
...
class FCRNet(BaseModule):
    def __init__(self,
        ...
        self.inc = nn.Sequential(
            DoubleConv(in_channels, channels[0],norm_cfg=norm_cfg,act_cfg=act_cfg),  # 第一层只有DoubleConv
            ConvModule(channels[0],channels[0],kernel_size=3,stride=1,padding=1,norm_cfg=norm_cfg,act_cfg=act_cfg)
        )       # b,32,512,512
        #
        ...
        self.conv45 = ConvModule(channels[3],channels[2],3,padding=1,norm_cfg=norm_cfg,act_cfg=act_cfg)
        ### 128,64,64
        ...
        self.conv56 = nn.Sequential(
            ...
            ACBlock(channels[3],channels[2],norm_cfg=norm_cfg,act_cfg=act_cfg,init_cfg=init_cfg),
