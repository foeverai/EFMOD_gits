...
from pytorch_wavelets import DTCWTForward,DWTForward,DTCWTInverse
from mmcv.cnn import ConvModule
from mmseg.registry import MODELS
from typing import Optional, Union, Sequence
# from ..utils import ResLayer
from mmseg.models.utils import ResLayer
from mmseg.models.utils import autopad, make_divisible
...
class DTCWT_downsample_layer(nn.Module):
    def __init__(self,
        ...
        self.dtcwt = DTCWTForward(J=j, biort='near_sym_a', qshift='qshift_a')
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)
        self.muti_ext = InceptionBottleneck(in_channels=13*in_cha,out_channels=out_cha)
    ...
    def forward(self, x):
        """
         ...
         ([1,3,h/2,w/2])
         yh
         torch.Size([1, 3, 6, h/2, w/2, 2])
         torch.Size([1, 3, 6, h/4, h/4, 2])
         torch.Size([1, 3, 6, 8, 8, 2])
         """
        yL, yH = self.dtcwt(x)
        yH_clamped = [yh for yh in yH]
        abs_ele = []
        num = len(yH)
        sum_list=[]
        # ronghe
        for i in range(num):
            real_part = yH_clamped[i][..., 0]
            imag_part = yH_clamped[i][..., 1]
            sum_ = real_part + imag_part
            abs_ele.append(sum_)
        for i in range(2):
            y_15 = abs_ele[i][:, :, 0, ::]
            y_45 = abs_ele[i][:, :, 1, ::]
            ...
            y_135 = abs_ele[i][:, :, 4, ::]
            y_165 = abs_ele[i][:, :, 5, ::]
            temp_out = torch.cat((y_15,y_45,y_75,y_105,y_135,y_165),dim=1)
            sum_list.append(temp_out)
        s1 = self.upsample (sum_list[1])
        out = torch.cat((sum_list[0],s1,yL),dim=1)
        out = self.muti_ext(out)
        return out
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        ...
        if self.deep_stem:
            self.stem = nn.Sequential(
                ...
                #     padding=1,
                #     bias=False),
                ConvModule(in_channels,stem_channels//2,3,1,1),
                build_norm_layer(self.norm_cfg, stem_channels // 2)[1],
                nn.ReLU(inplace=True),
                DTCWT_downsample_layer(stem_channels//2,stem_channels//2),
                build_conv_layer(
                    self.conv_cfg,
                    stem_channels // 2,
                    stem_channels // 2,
...
# @MODELS.register_module()
class ResNetV1c_Dtcwt(ResNet_Dtcwt):
...
# @MODELS.register_module()
class ResNetV1d_Dtcwt(ResNet_Dtcwt):
...
if __name__ == '__main__':
    ...
    block = DTCWT_downsample_layer(3,16).cuda()
