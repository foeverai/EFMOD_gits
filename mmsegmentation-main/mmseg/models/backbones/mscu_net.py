...
class MSCU_Net(BaseModule):
    # 2025.01.7 在整个网络中使用 旋转特征图的方向带状卷积方式
    #
    def __init__(self,
                 in_channels=3,
                 base_channels=32,
        ...
        super(MSCU_Net, self).__init__(init_cfg)
        ...
        # self.finalconv2 = ConvModule(channels[0], 32, 3, padding=1,norm_cfg=None,act_cfg=None)
    ...
    def forward(self, x):
        out = []
        x1 = self.init_block(x)         # (bs,64,h/2,w/2)
        # print(x1.shape)

        e1 = self.encoder1(x, x1)       # (bs,128,h/2,w/2)
        # print(e1.shape)
        e2 = self.encoder2(x, e1)       # (bs,256,h/4,w/4)
        e3 = self.encoder3(x, e2)       # (bs,512,h/8,w/8)
        e4 = self.encoder4(x, e3)       # (bs,1024,h/16,w/16)
        out.append(e4)                  # index=0
        # decoder_data1
        d_up1 = self.up4(e4)            # (bs,512,h/8，w/8)
        e0_in = self.conv12(x1)         # (bs,128,h/4,w/4)
        e0_in = self.conv13(e0_in)      # (bs,256,h/4,2/4)
        e0_in = self.conv14(e0_in)      # (bs,512,h/8,w/8)

        e1_in = self.conv13(e1)         # (bs,256,h/4,w/4)
        e1_in = self.conv14(e1_in)      # (bs,512,h/8,w/8)

        e2_in = self.conv14(e2)         # (bs,512,h/8,w/8)

        decoder_in = torch.cat((e0_in,e1_in,e2_in,e3,d_up1),dim=1)  # (bs,512*5,h/8,w/8)
        decoder4 = self.decoder4(self.skblock4(decoder_in))        # (bs,512,h/8,w/8)
        del d_up1,e0_in,e1_in,e2_in,decoder_in
        out.append(decoder4)            # index=1

        # decoder_data2
        d_up2 = self.up4(e4)            # (bs,512,h/8,w/8)
        d_up2 = self.up3(d_up2)         # (bs,256,h/4,w/4)

        e01_in = self.conv22(x1)        # (bs,128,h/2,w/2)
        e01_in = self.conv23(e01_in)    # (bs,256,h/4,w/4)

        e11_in = self.conv23(e1)        # (bs,256,h/4,w/4)

        decoder_in1 = self.up3(decoder4) # (bs,256,h/4,w/4)

        decoder_in = torch.cat((e01_in,e11_in,e2,decoder_in1,d_up2),dim=1)  # (bs,256*5,h/4,w/4)
        decoder3 = self.decoder3(self.skblock3(decoder_in))        # (bs,256,h/4,w/4)

        del d_up2,decoder_in,decoder_in1,e11_in,e01_in
        out.append(decoder3)            # index=2
        # decoder_data3
        d_up3 = self.up4(e4)            # (bs,512,h/8,w/8)
        d_up3 = self.up3(d_up3)         # (bs,256,h/4.w/4)
        d_up3 = self.up2(d_up3)         # (bs,128,h/2,w/2)

        e02_in = self.conv32(x1)        # (bs,128,h/2,w/2)

        decoder_in1 = self.up3(decoder4)           # (bs,256,h/4,w/4)
        decoder_in1 = self.up2(decoder_in1)        # (bs,128,h/2,w/2)

        decoder_in2 = self.up2(decoder3)        # (bs,128,h/2,w/2)

        decoder_in = torch.cat((e02_in,e1,decoder_in2,decoder_in1,d_up3),dim=1)
        decoder2 = self.decoder2(self.skblock2(decoder_in))     # (bs,128,h/2,w/2)
        del d_up3,e02_in,decoder_in1,decoder_in2,decoder_in
        out.append(decoder2)            # index=3
        # decoder_data4
        d_up4 = self.up4(e4)  # (bs,512,h/8,w/8)
        d_up4 = self.up3(d_up4)  # (bs,256,h/4.w/4)
        d_up4 = self.up2(d_up4)  # (bs,128,h/2,w/2)
        d_up4 = self.up1(d_up4)  # (bs,64,h,w)

        decoder_in1 = self.up3(decoder4)  # (bs,256,h/4,w/4)
        decoder_in1 = self.up2(decoder_in1)  # (bs,128,h/2,w/2)
        decoder_in1 = self.up1(decoder_in1)  # (bs,64,h/2,w/2)

        decoder_in2 = self.up2(decoder3)        # (bs,128,h/2,w/2)
        decoder_in2 = self.up1(decoder_in2)     # (bs,64,h,w)

        decoder_in3 = self.up1(decoder2)        # (bs,64,h,w)

        e1_in = self.up0(x1)                    # (bs,64,h,w)

        decoder_in = torch.cat((e1_in,decoder_in3,decoder_in2,decoder_in1,d_up4),dim=1)  # (bs,64*5,h,w)
        decoder1 = self.decoder1(self.skblock1(decoder_in))     # (bs,64,h,w)
