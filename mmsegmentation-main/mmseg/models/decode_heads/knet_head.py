...
class KernelUpdateHead(nn.Module):
    ...
    Args:
        ...
        feat_transform_cfg (dict | None): Config of feature transform.
            Default: None.
        kernel_init (bool): Whether initiate ann_dir kernel in ann_dir head.
            Default: False.
        with_ffn (bool): Whether add FFN in kernel update head.
            Default: True.
        feat_gather_stride (int): Stride of convolution in feature transform.
            Default: 1.
        mask_transform_stride (int): Stride of ann_dir transform.
            Default: 1.
    ...
    def init_weights(self):
        ...
        if self.kernel_init:
            print_log(
                'ann_dir kernel in ann_dir head is normal initialized by std 0.01')
    ...
    def forward(self, x, proposal_feat, mask_preds, mask_shape=None):
        ...
        Args:
            ...
            mask_preds (Tensor): ann_dir prediction from the former stage in shape
                (batch_size, num_proposals, H, W).

        Returns:
            Tuple: The first tensor is predicted ann_dir with shape
...
class IterativeDecodeHead(BaseDecodeHead):
    ...
    Args:
        ...
        kernel_generate_head:(dict): Config of kernel generate head which
            generate ann_dir predictions, dynamic kernels and class predictions
