...
class ResizeToMultiple(BaseTransform):
    """Resize img_dir & seg to multiple of divisor.
    ...
    Args:
        size_divisor (int): img_dir and gt seg maps need to resize to multiple
    ...
    def transform(self, results: dict) -> dict:
        """Call function to resize img_dir, semantic segmentation map to
...
class Rerange(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        """Call function to rerange img_dir.
...
class CLAHE(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        """Call function to Use CLAHE method process img_dir.
...
class RandomCrop(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        """Transform function to randomly crop img_dir, semantic segmentation
        maps.
...
class PhotoMetricDistortion(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        """Transform function to perform photometric distortion on img_dir.

        Args:
            results (dict): Result dict from loading pipeline.

        Returns:
            dict: Result dict with img_dir distorted.
        """
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        """Flip img_dir, bounding boxes and semantic segmentation map."""
        # flip image
...
class Resize(MMCV_Resize):
    ...
    Args:
        ...
        clip_object_border (bool): Whether to clip the objects
            ...
            bboxes are allowed to cross the border of img_dir. Therefore, we
...
class RandomMosaic(BaseTransform):
    """Mosaic augmentation. Given 4 img_dir, mosaic transform combines them into
    ...
    .. code:: text
     ...
     The mosaic transform steps are as follows:
         1. Choose the mosaic center as the intersections of 4 img_dir
         2. Get the left top image according to the index, and randomly
            sample another 3 img_dir from the custom dataset.
    ...
    Args:
        ...
        img_scale (Sequence[int]): Image size after mosaic pipeline of
            ...
            that of a single image. The output image comprises 4 single img_dir.
            Default: (640, 640).
    ...
    def _mosaic_combine(self, loc: str, center_position_xy: Sequence[float],
        ...
        Args:
            ...
            center_position_xy (Sequence[float]): Mixing center for 4 img_dir,
                (x, y).
...
class GenerateEdge(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        Returns:
            dict: Result dict with edge ann_dir.
        """
...
class ResizeShortestEdge(BaseTransform):
    """Resize the image and ann_dir while keeping the aspect ratio unchanged.
...
class BioMedical3DRandomCrop(BaseTransform):
    """Crop the input patch for medical image & segmentation ann_dir.

    Required Keys:

    - img (np.ndarray): Biomedical image with shape (N, Z, Y, X),
        N is the number of modalities, and data type is float32.
    - gt_seg_map (np.ndarray, optional): Biomedical semantic segmentation ann_dir
        with shape (Z, Y, X).
    ...
    def random_sample_location(self, seg_map: np.ndarray) -> dict:
        ...
        for c in all_classes:
            if c == 0:
                # to avoid the segmentation ann_dir full of background 0
...
class BioMedical3DPad(BaseTransform):
    ...
    def _pad_img(self, results: dict) -> None:
        """Pad img_dir according to ``self.pad_shape``
    ...
    def transform(self, results: dict) -> dict:
        """Call function to pad img_dir, semantic segmentation maps.
...
class BioMedical3DRandomFlip(BaseTransform):
    """Flip biomedical 3D img_dir and segmentations.
...
class Albu(BaseTransform):
    ...
    def __init__(self,
        ...
        if not keymap:
            self.keymap_to_albu = {'img': 'image', 'gt_seg_map': 'ann_dir'}
        else:
    ...
    def transform(self, results):
        ...
        # Convert to RGB since Albumentations works with RGB img_dir
        if self.bgr_to_rgb:
...
class ConcatCDInput(BaseTransform):
    """Concat img_dir for change detection.
