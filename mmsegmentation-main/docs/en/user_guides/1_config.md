...
data_preprocessor = dict(  # The config of data preprocessor, usually includes image normalization and augmentation.
    ...
    mean=[123.675, 116.28, 103.53],  # Mean values used for normalizing the input img_dir.
    std=[58.395, 57.12, 57.375],  # Standard variance used for normalizing the input img_dir.
...
train_pipeline = [  # Training pipeline.
    dict(type='LoadImageFromFile'),  # First pipeline to load img_dir from file path.
    dict(type='LoadAnnotations'),  # Second pipeline to load annotations for current image.
    dict(type='RandomResize',  # Augmentation pipeline that resize the img_dir and their annotations.
    ...
    dict(type='RandomFlip',  # Augmentation pipeline that flip the img_dir and their annotations
